...
import type { SeatProjection, BlocAnalysis } from './types';
import { PARTY_COLORS } from './visualization';
...
export function generateSeatProjectionChart(seatProjection: SeatProjection): string {
    const { projections, lookbackDays, pollCount, houses, date, totalSeats, threshold } = seatProjection;
...
export function generateBlocSummary(blocAnalysis: BlocAnalysis): string {
    const { blocs, lookbackDays, pollCount, majority } = blocAnalysis;
    
    const winner = blocs.red.hasMajority ? 'Rød-grønn' : 
                   blocs.blue.hasMajority ? 'Borgerlig' : 
                   'Ingen flertall';
